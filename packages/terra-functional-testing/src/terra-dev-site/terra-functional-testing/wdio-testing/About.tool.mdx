...
Terra exposes a `Terra.dispatchCustomEvent` which will dispatch a custom event with a metaData property; useful for edge scenarious that utilize a custom event listener to trigger some action.
