...
Fire a custom event with `metadata`. Use this to manually update a deployed terra application's locale, or fire updates for a custom event listener. This method accepts an object containing the event `name` (string) and event `metaData` (object).
...
it('should update an application's locale, () => {
  ...
  Terra.dispatchCustomEvent(event);
});
```
