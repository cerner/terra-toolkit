...
const log = logger('wdio-terra-visual-regression-service:BaseCompare');
...
export default class BaseCompare {
  ...
  getRuntimeConfigFileName(name) {
    ...
    const hash = require('crypto') // eslint-disable-line global-require
      .createHash('md5')
    ...
    const pid = !process.hasOwnProperty('ppid') ? null : runner ? process.ppid : process.pid; // eslint-disable-line no-prototype-builtins, no-nested-ternary
