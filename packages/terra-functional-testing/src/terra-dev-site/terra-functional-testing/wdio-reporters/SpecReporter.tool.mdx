...
The following section is only applicable if using the spec reporter standalone. The default `wdio.conf.js` is setup to automatically merge and output the result files.
...
The spec reporter offers utilities for merging multiple result files into a single file and cleaning up files. Merged result files will be generated using the following format:
