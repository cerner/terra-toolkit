...
| ignoreComparisonResults        | boolean | false    | false                   | Whether or not to disable test assertions on the screenshot comparison result.                |
...
```js
baseScreenshotDir/test_spec_directory/test_spec_name/__snapshots__/(reference|latest|diff)/theme/locale/browserName_terraViewportName/screenshot-name.png
```
