...
import saveElementScreenshot from '../../../../../../lib/services/wdio-terra-visual-regression/commands/saveElementScreenshot';
// Note: we import this that way to allow sinon to stub the default method
import * as makeElementScreenshot from '../../../../../../lib/services/wdio-terra-visual-regression/modules/makeElementScreenshot';
import * as saveBase64Image from '../../../../../../lib/services/wdio-terra-visual-regression/utils/saveBase64Image';
...
describe('saveElementScreenshot', function() {
  ...
  it('works correctly when called with elementSelector only', async function() {
    ...
    const {
      args: [browser, elementSelector, options],
    } = this.makeElementScreenshot.firstCall;
    assert.strictEqual(browser, this.browserMock, 'makeElementScreenshot 1st parameter should be browser');
    assert.strictEqual(
      elementSelector,
      this.elementSelector,
      'makeElementScreenshot 2nd parameter should be elementSelector',
    );
  ...
  it('works correctly when called with fileName, elementSelector, options', async function() {
    // check saveElementScreenshot
    const base64String = await saveElementScreenshot.call(
      this.browserMock,
      this.fileName,
      this.elementSelector,
      this.options,
    ...
    const {
      args: [fileName, base64Image],
    } = this.saveBase64Image.firstCall;
