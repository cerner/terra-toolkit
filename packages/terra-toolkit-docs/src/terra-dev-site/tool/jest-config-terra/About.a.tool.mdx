...
* Maps css, scss, and svg files to the [identity-obj-proxy](https://github.com/keyz/identity-obj-proxy).
* Maps terra-aggregate-translations locale files to a mock implementation.


## What is Jest

[Jest](https://eslint.org/) is a "Jest is a delightful JavaScript Testing Framework with a focus on simplicity".
...
## Usage
To extend `jest-config-terra` add a `jest.config.js` file to the root of your project, require `@cerner/jest-config-terra` and export the config.
