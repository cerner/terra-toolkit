...
- [DefinePlugin](https://webpack.js.org/plugins/define-plugin/) - Plugin to define global compile-time values, including:
  ...
  - `TERRA_THEME_CONFIG` - The scoped and default theme. See [theme config](/dev_tools/terra-toolkit-docs/webpack-config-terra/theme-config) for more details about the object structure.
