...
module.exports = {
  lint: ({ packageJsonData, ruleConfig }) => {
    ...
    if (currentProblems.length) {
      const lintMessage = `The dependencies for this project do not have the minimum versions required for no terra base peer dependencies:\n  ${currentProblems.join('\n  ')}`;
      return new LintIssue({
        lintId: 'require-no-terra-base-peer-dependency-versions', severity: ruleConfig.severity, node: 'dependencies', lintMessage,
      });
    }
    return undefined;
  },
