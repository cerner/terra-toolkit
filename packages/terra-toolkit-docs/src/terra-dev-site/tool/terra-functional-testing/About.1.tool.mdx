...
Test utilities are available to help write tests. The utilities are accessed using the `Terra` global object.

## Describe Helpers

The describe helpers are an alias for the top level `describe` block used within spec files. The describe helpers provide useful features for restricting a set of tests to specific form factors, locales, and themes.
...
The following example will scope the tests to run only for the `tiny` and `huge` form factors.
