...
const getSpecResults = (directory) => {
  const specResults = fs.readdirSync(directory).filter((file) => (
    file.indexOf('wdio-spec-results') > -1
  ));
...
const getResultsFileName = (options) => {
  ...
  const keys = [locale, theme, formFactor, browser, packageName].filter(key => !!key);
...
const mergeSpecResults = (results) => {
  ...
  results.forEach((result) => {
    const { capabilities, packageName, ...spec } = result;
    const { browserName } = capabilities;

    if (!browsers[browserName]) {
      browsers[browserName] = { capabilities, packages: {} };
    }

    if (!browsers[browserName].packages[packageName]) {
      browsers[browserName].packages[packageName] = { specs: [] };
    }
...
const writeResultsToFile = (outputDir, results, options) => {
  const { theme, locale, formFactor } = options;

  Object.keys(results).forEach((browser) => {
    const { capabilities, packages } = results[browser];
    ...
    Object.keys(packages).forEach((packageName) => {
      ...
      const fileName = path.join(outputDir, getResultsFileName({ ...options, browser, packageName }));
