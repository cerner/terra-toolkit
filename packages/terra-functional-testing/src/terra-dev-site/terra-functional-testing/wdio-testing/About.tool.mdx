...
### Terra.setLocale
Use this to manually update a terra application's locale on a case by case basis. Particularly useful for testing locale changes on a deployed application. This method accepts a string containing the `locale` (string) to update to.
